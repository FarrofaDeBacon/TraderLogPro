<script lang="ts">
    ...
    async function handleSave() {
        ...
        if (isNaN(resParsed)) {
            toast.error("Resultado numérico inválido.");
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                toast.success("Edição Expressa salva! ⚡");
                open = false;
                onsave();
            } else {
            ...
            toast.error("Falha de rede ao salvar edição rápida.");
        } finally {
            isSubmitting = false;
        }
    }
    ...
    <Dialog.Content class="max-w-sm glass border-white/10 text-white p-6 shadow-2xl">
        <Dialog.Header>
            <Dialog.Title class="flex items-center gap-2">
                ...
                {$t("trades.quickEdit.title") || "Edição Expressa"}
            </Dialog.Title>
            <Dialog.Description class="text-muted-foreground text-xs pt-1">
                Corrija o resultado bruto sem abrir o Wizard Completo.
            </Dialog.Description>
        </Dialog.Header>
        ...
        {#if trade}
            <div class="space-y-4 mt-2">
                <div class="space-y-1.5 relative">
                    <span class="text-[9px] font-black tracking-widest text-muted-foreground uppercase ml-1 block mb-1">Data e Hora</span>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-1.5 relative">
                        <span class="text-[9px] font-black tracking-widest text-muted-foreground uppercase ml-1 block mb-1">Ativo</span>
                        <Input bind:value={asset} class="bg-zinc-900/50 border-dashed border-border/30 uppercase font-mono font-bold h-10" />
                    </div>
                    
                    <div class="space-y-1.5 relative">
                        <span class="text-[9px] font-black tracking-widest text-muted-foreground uppercase ml-1 block mb-1">Viés</span>
                ...
                <div class="space-y-1.5 relative">
                    <span class="text-[9px] font-black tracking-widest text-muted-foreground uppercase ml-1 block mb-1">Resultado (R$)</span>
                ...
                <div class="flex items-center justify-between pt-4 mt-2 border-t border-border/10">
                    ...
                    <Button disabled={isSubmitting} variant="default" class="bg-primary text-primary-foreground font-black tracking-widest uppercase text-[10px] h-8 px-4" onclick={handleSave}>
                        {#if isSubmitting} <Loader2 class="w-4 h-4 animate-spin" /> {:else} Salvar {/if}
                    </Button>
                </div>
            </div>
        {/if}
