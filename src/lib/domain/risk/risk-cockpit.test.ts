...
import type { RiskProfileConfig, TradeRiskSnapshot, GrowthPhase } from './types';
...
describe('Domain: Risk Cockpit', () => {
    ...
    it('deve agregar os status financeiros e comportamentais sem motor de growth caso o capital inicial não seja preenchido', () => {
        const trades = [createTestTrade(20)];

        const state = buildRiskCockpitState(baseProfile, trades);
    ...
    it('deve agregar todos os três módulos caso a growth phase e o capital sejam supridos', () => {
        const trades = [createTestTrade(200), createTestTrade(-50)];

        const startingCapital = 1000;
        const state = buildRiskCockpitState(baseProfile, trades, targetPhase, startingCapital);

        expect(state.dailyRiskStatus.dailyPnL).toBe(150); // 200 - 50 = +150 today
        expect(state.disciplineEvaluation.score).toBe(100); 
